...
const databaseSchema = `{
  users: {
    "{studentId}": {
      studentId, password, name, role: "admin" | "user",
      status: "pending" | "approved" | "rejected",
      college, department, phone, requestedAt?, approvedAt?, tempPasswordIssuedAt?
    }
  },
  rooms: {
...
function App() {
  ...
  async function signup(event) {
    ...
    const user = { ...signupForm, role: "user", status: "pending", requestedAt: new Date().toISOString() };
    await persist(`/users/${signupForm.studentId}`, user, "회원가입 요청이 접수되었습니다. 관리자 승인 후 로그인할 수 있습니다.");
    setSignupForm({ studentId: "", password: "", name: "", college: ALLOWED_COLLEGE, department: "", phone: "" });
    setAuthMode("login");
  }
  ...
  return (
    <section className="auth-layout">
      ...
      <div className="home-actions panel">
        ...
        <p className="intro-question">팀플이 있는데 공간이 없다면?</p>
      ...
      {authMode && (
        <Modal title={authMode === "login" ? "로그인" : "회원가입"} onClose={() => setAuthMode(null)}>
          <div className="auth-panel modal-panel">
...
function ApprovalsAdmin({ data, patch }) {
  const users = objectList(data.users).filter((user) => user.role !== "admin");
  ...
  return (
    <Panel title="회원가입 승인 관리" icon={UserRound}>
      ...
      <DataTable headers={["상태", "학번", "이름", "소속대학", "소속학과", "전화번호", "관리"]}>
        {users.map((user) => (
...
function UserDashboard({ data, user, persist, patch, drop, notify, onUserUpdate }) {
  ...
  const [profileForm, setProfileForm] = useState({
    name: user.name || "",
    college: user.college || ALLOWED_COLLEGE,
  ...
  async function saveProfile(event) {
    event.preventDefault();
    if (profileForm.college !== ALLOWED_COLLEGE) {
      notify("인문사회과학대학 소속 학생만 이용할 수 있습니다.");
      return;
    }
      ...
      {activeModal === "profile" && (
        <Modal title="내 정보 확인 및 수정" onClose={() => setActiveModal(null)}>
          <form className="form-grid" onSubmit={saveProfile}>
            ...
            <label>이름<input value={profileForm.name} onChange={(event) => setProfileForm({ ...profileForm, name: event.target.value })} required /></label>
            <label>소속대학<input value={profileForm.college} onChange={(event) => setProfileForm({ ...profileForm, college: event.target.value })} required /></label>
            <label>소속학과<input value={profileForm.department} onChange={(event) => setProfileForm({ ...profileForm, department: event.target.value })} required /></label>
