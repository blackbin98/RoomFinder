...
function getRoomFloor(roomName) {
  const match = String(roomName || "").match(/C25-(\d+)/);
  if (!match) return "";
  const number = match[1];
  const floor = number.length >= 4 ? Number(number.slice(0, 2)) : Number(number[0]);
  return `${floor}층`;
}
...
function getNextSchedule(data, roomId, date, fromMinutes) {
  const day = dayName(date);
  return objectList(data.schedules)
    .filter((schedule) => schedule.roomId === roomId && schedule.day === day && minutesFromTime(schedule.startTime) > fromMinutes)
    .sort((a, b) => minutesFromTime(a.startTime) - minutesFromTime(b.startTime))[0];
}

function getRoomStatus(data, room, date, periodNumber, now) {
  const currentDate = dateInputFrom(now);
  const selectedDay = dayName(date);
  const currentMinutes = minutesNow(now);
  const selectedPeriod = getPeriod(periodNumber);
  const selectedStart = minutesFromTime(selectedPeriod.start);
  const activeSchedule = objectList(data.schedules).find((schedule) => (
    schedule.roomId === room.id &&
    schedule.day === selectedDay &&
    Number(schedule.period) === Number(periodNumber)
  ));
  const activeReservation = objectList(data.reservations).find((reservation) => (
    reservation.roomId === room.id &&
    reservation.date === date &&
    Number(reservation.period) === Number(periodNumber)
  ));
  const nextSchedule = getNextSchedule(data, room.id, date, selectedStart);
  const nextStart = nextSchedule ? minutesFromTime(nextSchedule.startTime) : null;

  if (activeSchedule) {
    return { state: "busy", label: "수업 중", detail: activeSchedule.subject, schedule: activeSchedule, reservation: null, nextSchedule };
  }
  if (activeReservation) {
    return { state: "reserved", label: "예약됨", detail: `${activeReservation.startTime}~${activeReservation.endTime}`, schedule: null, reservation: activeReservation, nextSchedule };
  }
  if (date === currentDate && selectedPeriod.period === getCurrentPeriod(now)?.period && nextSchedule && nextStart - currentMinutes <= 10 && nextStart - currentMinutes >= 0) {
    return { state: "soon", label: "곧 수업 시작", detail: `다음 수업 ${nextSchedule.startTime}`, schedule: null, reservation: null, nextSchedule };
  }
  return {
    state: "available",
    label: "사용 가능",
    detail: nextSchedule ? `다음 수업 ${nextSchedule.startTime}` : "오늘 남은 수업 없음",
    schedule: null,
    reservation: null,
    nextSchedule
  };
  ...
  const features = [
    {
      icon: Search,
      title: "실시간 조회",
      body: "현재 사용 가능한 강의실을 한눈에 확인"
    },
    {
      icon: CalendarDays,
      title: "간편 예약",
      body: "정규 수업 시간을 피해 1시간 단위 예약"
    },
    {
      icon: BookOpen,
      title: "강의 정보 확인",
      body: "과목명, 담당교수, 학과 정보 제공"
    }
  ];
  ...
  return (
    <section className="landing-page">
      <div className="landing-hero">
        <span className="landing-pill">부경대학교 C25 시범 운영</span>
        <h2>팀플이 있는데 공간이 없다면?</h2>
        <p>RoomFinder로 C25 빈 강의실을 실시간으로 확인하고 바로 예약하세요.</p>
        <div className="landing-actions">
          <button className="primary-button landing-button" onClick={() => setAuthMode("login")}><Shield size={20} /> 로그인</button>
          <button className="secondary-button landing-button" onClick={() => setAuthMode("signup")}><UserRound size={20} /> 회원가입</button>
        </div>
      </div>
...
function UserDashboard({ data, user, persist, patch, drop, notify, onUserUpdate }) {
  ...
  const rooms = objectList(data.rooms);
  const roomStatuses = useMemo(() => rooms.map((room) => ({
    room,
    floor: getRoomFloor(room.name),
    status: getRoomStatus(data, room, date, Number(period), now)
  })), [rooms, data, date, period, now]);
  const floorOptions = useMemo(() => {
    const floors = [...new Set(rooms.map((room) => getRoomFloor(room.name)).filter(Boolean))];
    return ["전체", ...floors.sort((a, b) => Number(a.replace("층", "")) - Number(b.replace("층", "")))];
  }, [rooms]);
  const reservationOccupancy = useMemo(() => getOccupancy(data, date, period), [data, date, period]);
  const liveAvailableRooms = roomStatuses.filter((item) => item.status.state === "available");
  const liveBusyRooms = roomStatuses.filter((item) => item.status.state === "busy");
  const liveReservedRooms = roomStatuses.filter((item) => item.status.state === "reserved");
  const availableRooms = rooms.filter((room) => !reservationOccupancy[room.id]);
  ...
  const filteredRooms = roomStatuses
    .filter((item) => floorFilter === "전체" || item.floor === floorFilter)
    .filter((item) => statusFilter === "전체" || item.status.label === statusFilter)
    .sort((a, b) => {
      const order = { available: 0, soon: 1, reserved: 2, busy: 3 };
      return (order[a.status.state] ?? 9) - (order[b.status.state] ?? 9) || a.room.name.localeCompare(b.room.name, "ko", { numeric: true });
    });
  useEffect(() => {
    setProfileForm({
      password: user.password || "",
  ...
  useEffect(() => {
    if (!floorOptions.includes(floorFilter)) setFloorFilter("전체");
  ...
  async function reserve(event) {
    ...
    if (reservationOccupancy[roomId]) {
  ...
  function openReserveFor(room) {
    setRoomId(room.id);
    setDate(currentDate);
    if (currentPeriod) setPeriod(currentPeriod.period);
  ...
  function openFirstAvailableRoom() {
    const first = filteredRooms.find((item) => item.status.state === "available");
    if (!first) {
      notify("현재 바로 예약 가능한 강의실이 없습니다.");
      return;
    }
    openRoomDetail(first.room);
  }

  function selectCalendarDate(day) {
    if (!day) return;
    const value = dateInputFrom(day);
    if (value < currentDate) return;
    setDate(value);
  }

  const selectedRoomStatus = selectedRoom ? getRoomStatus(data, selectedRoom, date, Number(period), now) : null;
  const selectedRoomCanReserve = selectedRoomStatus?.state === "available";

  return (
    <section className="user-grid">
      <div className="user-action-bar">
        <button className="primary-button" onClick={openFirstAvailableRoom}><CalendarDays size={18} /> 예약하기</button>
      ...
      <Panel title="예약하기" icon={CalendarDays}>
        <section className="booking-dashboard">
          <div className="booking-time-card">
            <span>현재 날짜</span>
            <strong>{formatKoreanDate(currentDate)}</strong>
          </div>
          <div className="booking-time-card">
            <span>현재 시각</span>
          ...
          <div className="booking-time-card">
            <span>선택 시간</span>
            <strong>{getPeriod(period).label} {getPeriod(period).slot}</strong>
          </div>
          <div className="booking-counts">
            <article className="summary-card available"><span>사용 가능</span><strong>{liveAvailableRooms.length}</strong></article>
            <article className="summary-card busy"><span>수업 중</span><strong>{liveBusyRooms.length}</strong></article>
            <article className="summary-card reserved"><span>예약됨</span><strong>{liveReservedRooms.length}</strong></article>
          </div>
        </section>

        <section className="calendar-panel">
          <div className="calendar-head">
            <button className="icon-button" type="button" onClick={() => setCalendarMonth(new Date(calendarMonth.getFullYear(), calendarMonth.getMonth() - 1, 1))}><ChevronLeft size={18} /></button>
            <strong>{calendarMonth.getFullYear()}년 {calendarMonth.getMonth() + 1}월</strong>
            <button className="icon-button" type="button" onClick={() => setCalendarMonth(new Date(calendarMonth.getFullYear(), calendarMonth.getMonth() + 1, 1))}><ChevronRight size={18} /></button>
          </div>
          ...
          <div className="calendar-grid">
            {calendarDays.map((day, index) => {
              const value = day ? dateInputFrom(day) : "";
              const isPast = value && value < currentDate;
              return (
                <button
                  key={`${value}-${index}`}
                  className={`${value === currentDate ? "today" : ""} ${value === date ? "selected" : ""}`}
                  type="button"
                  disabled={!day || isPast}
          ...
          <p className="calendar-selected">선택 날짜: <strong>{formatKoreanDate(date)}</strong></p>
        </section>

        <section className="booking-filters">
          <label>시간 선택<select value={period} onChange={(event) => setPeriod(Number(event.target.value))}>
            {PERIODS.map((item) => <option key={item.period} value={item.period}>{item.label} {item.slot}</option>)}
          </select></label>
          <label>층별 필터<select value={floorFilter} onChange={(event) => setFloorFilter(event.target.value)}>
            {floorOptions.map((floor) => <option key={floor}>{floor}</option>)}
          </select></label>
          <label>상태 필터<select value={statusFilter} onChange={(event) => setStatusFilter(event.target.value)}>
            {["전체", "사용 가능", "수업 중", "예약됨", "곧 수업 시작"].map((status) => <option key={status}>{status}</option>)}
          </select></label>
        </section>

        <div className="compact-room-grid">
          {filteredRooms.map(({ room, status }) => (
            <button className={`compact-room-card ${status.state}`} key={room.id} type="button" onClick={() => openRoomDetail(room)}>
              <strong>{room.name}</strong>
              <span>{status.state === "available" ? "●" : status.state === "busy" ? "●" : status.state === "reserved" ? "●" : "●"} {status.label}</span>
              <p>{status.detail}</p>
            </button>
          ))}
        </div>
      ...
      {activeModal === "roomDetail" && selectedRoom && (
        <Modal title="강의실 상세" onClose={() => setActiveModal(null)}>
          <div className="room-detail">
            <div><span>강의실</span><strong>{selectedRoom.name}</strong></div>
            <div><span>선택 시간 상태</span><strong className={`detail-state ${selectedRoomStatus.state}`}>{selectedRoomStatus.label}</strong></div>
            <div><span>선택 날짜</span><strong>{date}</strong></div>
            <div><span>선택 시간</span><strong>{getPeriod(period).label} {getPeriod(period).slot}</strong></div>
            <div><span>다음 수업</span><strong>{selectedRoomStatus.nextSchedule ? `${selectedRoomStatus.nextSchedule.startTime}~${selectedRoomStatus.nextSchedule.endTime}` : "없음"}</strong></div>
            <div><span>과목명</span><strong>{selectedRoomStatus.schedule?.subject || selectedRoomStatus.nextSchedule?.subject || "-"}</strong></div>
            <div><span>담당교수</span><strong>{selectedRoomStatus.schedule?.professor || selectedRoomStatus.nextSchedule?.professor || "-"}</strong></div>
            <div><span>개설학과</span><strong>{selectedRoomStatus.schedule?.department || selectedRoomStatus.nextSchedule?.department || "-"}</strong></div>
            <div><span>예약 가능 시간</span><strong>{selectedRoomStatus.nextSchedule ? `${selectedRoomStatus.nextSchedule.startTime} 이전` : "현재 이후"}</strong></div>
          </div>
          {selectedRoomCanReserve ? (
            <form className="form-grid detail-reserve-form" onSubmit={reserve}>
              <label>예약 시간<select value={period} onChange={(event) => setPeriod(Number(event.target.value))}>{PERIODS.map((item) => <option key={item.period} value={item.period}>{item.label} {item.slot}</option>)}</select></label>
              <label>예약 목적<input placeholder="팀 프로젝트, 스터디, 발표 연습, 동아리 활동" value={purpose} onChange={(event) => setPurpose(event.target.value)} required /></label>
              <button className="primary-button"><CheckCircle2 size={18} /> 예약하기</button>
              <p className="hint">선택한 날짜의 예약 시간: {dailyHours}/2시간</p>
            </form>
          ) : (
            <button className="secondary-button" disabled>예약 불가</button>
          )}
        </Modal>
      )}
