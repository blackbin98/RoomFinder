...
function getRoomStatus(data, room, date, now) {
  const currentDate = dateInputFrom(now);
  const selectedDay = dayName(date);
  const currentMinutes = minutesNow(now);
  const activePeriod = getCurrentPeriod(now);
  const activePeriodNumber = activePeriod?.period;
  const activeSchedule = objectList(data.schedules).find((schedule) => (
    schedule.roomId === room.id &&
    schedule.day === selectedDay &&
    Number(schedule.period) === Number(activePeriodNumber)
  ));
  const activeReservation = objectList(data.reservations).find((reservation) => (
    reservation.roomId === room.id &&
    reservation.date === date &&
    Number(reservation.period) === Number(activePeriodNumber)
  ));
  const nextSchedule = getNextSchedule(data, room.id, date, currentMinutes);
  ...
  if (date === currentDate && nextSchedule && nextStart - currentMinutes <= 10 && nextStart - currentMinutes >= 0) {
...
function UserDashboard({ data, user, persist, patch, drop, notify, onUserUpdate }) {
  ...
  const roomStatuses = useMemo(() => rooms.map((room) => ({
    ...
    status: getRoomStatus(data, room, date, now)
  })), [rooms, data, date, now]);
  const liveOccupancy = useMemo(() => currentPeriod ? getOccupancy(data, date, currentPeriod.period) : {}, [data, date, currentPeriod]);
  ...
  const filteredRooms = roomStatuses
    ...
    .sort((a, b) => {
      if (sortMode === "room") return a.room.name.localeCompare(b.room.name, "ko", { numeric: true });
  ...
  const selectedRoomStatus = selectedRoom ? getRoomStatus(data, selectedRoom, date, now) : null;
    ...
    <section className="user-grid">
      ...
      <Panel title="예약하기" icon={CalendarDays}>
        ...
        <section className="booking-filters">
          ...
          <label>정렬<select value={sortMode} onChange={(event) => setSortMode(event.target.value)}>
            <option value="available">사용 가능 우선</option>
            <option value="room">강의실 번호순</option>
          </select></label>
        </section>
