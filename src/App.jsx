  ...
  return (
    <section className="auth-layout">
      ...
      <div className="home-actions panel">
        ...
        <p>C25 건물의 빈 강의실을 확인하고 필요한 시간에 예약할 수 있습니다.</p>
  ...
  return (
    <Panel title="강의편람 엑셀 업로드" icon={Upload}>
      ...
      >
        ...
        <button className="secondary-button" type="button" onClick={confirmFile}><Search size={16} /> 파일 확인</button>
      </div>
      {preview.length > 0 && (
        <>
          <div className="table-toolbar">
            <strong>미리보기 {preview.length}개 · 저장 가능 {preview.filter((row) => row.valid).length}개</strong>
            <button className="primary-button" onClick={applyPreview}><CheckCircle2 size={16} /> 반영하기</button>
          </div>
          <DataTable headers={["시트", "행", "상태", "과목명", "교수", "학과", "강의실", "요일", "교시", "오류"]}>
            {preview.map((row) => (
...
function UserDashboard({ data, user, persist, drop, notify }) {
  ...
  return (
    <section className="user-grid">
      ...
      {activeModal === "profile" && (
        <Modal title="내 정보 확인" onClose={() => setActiveModal(null)}>
