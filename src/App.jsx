...
function UserDashboard({ data, user, persist, patch, drop, notify, onUserUpdate }) {
  ...
  const rooms = objectList(data.rooms);
  const liveOccupancy = useMemo(() => currentPeriod ? getOccupancy(data, currentDate, currentPeriod.period) : {}, [data, currentDate, currentPeriod]);
  const reservationOccupancy = useMemo(() => getOccupancy(data, date, period), [data, date, period]);
  const liveAvailableRooms = rooms.filter((room) => !liveOccupancy[room.id]);
  const liveBusyRooms = rooms.filter((room) => liveOccupancy[room.id]?.type === "schedule");
  const liveReservedRooms = rooms.filter((room) => liveOccupancy[room.id]?.type === "reservation");
    ...
    <section className="user-grid">
      ...
      <Panel title="C25 전체 강의실 조회" icon={Search}>
        <div className="live-status-bar">
          ...
          <div>
            <span>조회 기준</span>
            <strong>{currentPeriod ? `${currentPeriod.label} ${currentPeriod.slot}` : "수업 시간 외"}</strong>
          </div>
          <div>
            <span>오늘</span>
        ...
        <div className="room-summary">
          <article className="summary-card available"><span>현재 예약 가능</span><strong>{liveAvailableRooms.length}</strong></article>
          <article className="summary-card busy"><span>현재 수업 중</span><strong>{liveBusyRooms.length}</strong></article>
          <article className="summary-card reserved"><span>현재 예약됨</span><strong>{liveReservedRooms.length}</strong></article>
        </div>
        <div className="room-grid">
          {rooms.map((room) => {
            const use = liveOccupancy[room.id];
            ...
            return (
              <article className={`room-card ${state}`} key={room.id}>
                ...
                <p>{state === "available" ? `${room.capacity}석 · 현재 비어 있습니다.` : use.type === "schedule" ? `${use.subject} · ${use.professor} · ${use.department}` : `${use.purpose} · ${use.userId}`}</p>
                {state === "available" ? (
      ...
      {activeModal === "reserve" && (
        <Modal title="강의실 예약" onClose={() => setActiveModal(null)}>
          <div className="calendar-summary">
            ...
            <div><span>현재 시각</span><strong>{formatClock(now)}</strong></div>
            <div><span>현재 교시</span><strong>{currentPeriod ? `${currentPeriod.label} ${currentPeriod.slot}` : "수업 시간 외"}</strong></div>
          </div>
